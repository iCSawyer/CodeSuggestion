...
def get_ground_truth_tokens(args, file_type):
    data_file = os.path.join(args.data_dir, args.dataset, f"{file_type}.jsonl")
    with open(data_file) as f:
        lines = f.readlines()
    list_tokens = [json.loads(x.strip())["context"].split(" ") for x in lines]
    return list_tokens
...
def evaluate():
    ...
    full_length_gt_tokens = get_ground_truth_tokens(args, "test")
    ...
    with torch.no_grad():
        for batch, item in enumerate(tqdm(eval_dataloader)):
            ...
            _tokens.append((pred_tokens, tgt_tokens))
...
with open("__temp__.json", "w") as f:
    json.dump(_tokens, f)
