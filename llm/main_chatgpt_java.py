...
def generate_prompt_chatgpt(
    ...
    for i in range(len(exemplars)):
        if exemplars[i].find("{") != -1:
            new_exemplars.append(exemplars[i])
    exemplars = new_exemplars

    for i in range(n_shot):
        idx_brace = exemplars[i].index("{")
        header = exemplars[i][:idx_brace].strip()
        body = exemplars[i][idx_brace:].strip()[:length]
...
if __name__ == "__main__":
    ...
    conf.log_name: str = f"./logs/chatgpt/java_{conf.template_index}_{conf.search_tool}"
    ...
    context_path = "context_dataset/test_java.jsonl"
